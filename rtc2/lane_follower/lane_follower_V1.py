...
class LineFollower(Node):
    ...
    def on_image(self, msg: Image):
        ...
        cv2.waitKey(1)
        h, w, _ = frame.shape
        offset_y = int(80)  # nach unten verschieben, da Kamera oben am TB3

        # ROI
        y0 = int(h * self.roi_y_start_ratio)- offset_y
        roi = frame[y0:(h - offset_y), :]
        cv2.imshow('TB3 Lane Follower Debug 2', roi)
        cv2.waitKey(1)
